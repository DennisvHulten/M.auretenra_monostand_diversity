Notebook for the genomic analysis of *Madracis auretenra* (DArTseq) to
accompany (van Hulten, in prep.)

Data was collected during two expeditions to Curacao, following a first
in 2021 when 493 samples were sequenced, and second in 2023 when 184
samples were collected and sequenced. The total dataset contains 681
individuals. All samples were shipped to Diversity Arrays Technology in
Canberra, Australia for sequencing (DArT seq). Backups of the original
samples are held at the California Academy of Sciences, San Francisco,
USA.
...
**Any scripts used in this analysis listed as
/home/pbongaerts/Github/radseq/ can be found on
...
![](media/image22.png){width="6.5in" height="2.7083333333333335in"}

## ![](media/image5.png){width="6.5in" height="2.7083333333333335in"}

## 

![](media/image57.png){width="6.5in" height="2.875in"}

Most of my sequences looks like this, they are supposed to run
parallel... most sequences look similar in the position of the peaks but
differ on peak intensity, this could be biologically relevant but could
also be contamination?

![](media/image53.png){width="6.5in" height="2.7083333333333335in"}

## ![](media/image39.png){width="6.5in" height="2.7083333333333335in"}

And warnings for sequence length dist....

![](media/image47.png){width="6.5in" height="2.7083333333333335in"}

![](media/image9.png){width="6.5in" height="2.7083333333333335in"}

![](media/image33.png){width="6.5in" height="9.598958880139982in"}
...
# B1_ipyrad_reference_assembly

%ipyrad -n MADR_reference

%nano params-MADR_reference

#MADR_REF = reference sequence from Sanger institute: NCBI; BioProject
[[PRJEB76086]{.underline}](https://www.ncbi.nlm.nih.gov/bioproject/PRJEB76086/)

\-\-\-\-\-\-- ipyrad params file
(v.0.9.102)\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\--

MADR_REF \## \[0\] \[assembly_name\]: Assembly name. Used to name output
directories for assembly steps

/home/dvanhulten/Projects/MADR/B2_reference_assembly \## \[1\]
\[project_dir\]: Project dir (made in curdir if not present)

\## \[2\] \[raw_fastq_path\]: Location of raw non-demultiplexed fastq
files
...
reference \## \[5\] \[assembly_method\]: Assembly method (denovo,
reference)

\~/Projects/MADR/B2_reference_assembly/madr_auretenra_ref.fasta \##
\[6\] \[reference_sequence\]: Location of reference sequence file

ddrad \## \[7\] \[datatype\]: Datatype (see docs): rad, gbs, ddrad, etc.

TGCAG, \## \[8\] \[restriction_overhang\]: Restriction overhang (cut1,)
or (cut1, cut2)

5 \## \[9\] \[max_low_qual_bases\]: Max low quality base calls (Q\<20)
in a read

33 \## \[10\] \[phred_Qscore_offset\]: phred Q score offset (33 is
default and very standard)

6 \## \[11\] \[mindepth_statistical\]: Min depth for statistical base
calling

6 \## \[12\] \[mindepth_majrule\]: Min depth for majority-rule base
calling
...
0 \## \[15\] \[max_barcode_mismatch\]: Max number of allowable
mismatches in barcodes

2 \## \[16\] \[filter_adapters\]: Filter for adapters/primers (1 or
2=stricter)
...
0.05 \## \[19\] \[max_Ns_consens\]: Max N\'s (uncalled bases) in
consensus
...
0.5 \## \[24\] \[max_shared_Hs_locus\]: Max \# heterozygous sites per
locus

0, 0, 0, 0 \## \[25\] \[trim_reads\]: Trim raw read edges (R1\>, \<R1,
R2\>, \<R2) (see docs)

0, 0, 0, 0 \## \[26\] \[trim_loci\]: Trim locus edges (see docs) (R1\>,
\<R1, R2\>, \<R2)
...
\--profile=\"MADR_ipyrad_reference\"; sleep 60

\% ipyrad -p params-MADR_reference.txt -s1234567 -c 30 \--ipcluster
MADR_ipyrad_reference
...
# C2_removing_symbiont_contamination

#pyrad2fasta.py from
[[https://github.com/pimbongaerts/radseq]{.underline}](https://github.com/pimbongaerts/radseq)

\%
/home/pbongaerts/Github/radseq/[[pyrad2fasta.py]{.underline}](http://pyrad2fasta.py)
...
\"\$INPUT_FASTA\" \>
\"\${OUTPUT_DIR}/MADR_denovo_to\_\${GENOME_SHORT}.sam\"

/home/pbongaerts/Github/radseq/mapping_get_bwa_matches.py
\"\${OUTPUT_DIR}/MADR_denovo_to\_\${GENOME_SHORT}.sam\" \>
\"\${OUTPUT_DIR}/MADR_denovo_to\_\${GENOME_SHORT}\_q20.txt\"

echo \"Processing for \${GENOME} completed.\"

done

cat bwa_outputs/MADR_denovo\_\*.txt \| cut -f1 \| sort \| uniq \>
...
-out \"**blastn_outputs/**MADR_reference_to\_\${GENOME_SHORT}.txt\"
...
\"**blastn_outputs/**MADR_reference_to\_\${GENOME_SHORT}.txt\"
\$MAX_E_VALUE
...
-db ../blast_db_12_26_2023/nt -task blastn -evalue 0.0001
-max_target_seqs 10 -outfmt \"7 qseqid sseqid length nident pident
evalue bitscore staxids stitle\" -out
...
MADR_reference_c3_temp.vcf MADR_reference_all_loci_to_remove.txt \>
MADR_reference_c3.vcf



# D1_initial_genetic_assessment

### D1a_removal_of_underperforming_ind_and_snps
...
indv_to_remove_reference_C3.txt \--max-missing 0.8 \--mac 1 \--minDP 8
\--recode \--stdout \> MADR_reference_d1_strict.vcf
...
MADR_reference_performance_d1_temp.txt
...
![](media/image63.png){width="6.505208880139983in"
...
MADR_sample_record.csv cur_annotations_full.csv \>
cur_annotations_temp.csv



For figures, see Tree_figures_d1.R

![](media/image72.png){width="6.5in" height="6.847222222222222in"}

![](media/image45.png){width="6.5in" height="6.694444444444445in"}

### D1c_genetic_structure
...
MADR_reference_popfile_d1.txt -o MADR_reference_d1_clones.txt
...
Based on these results we can put a threshold around 98% similarity for
our initial clone detection

python3 /home/pbongaerts/Github/radseq/detect_clones_vcf.py -v
MADR_reference_d1.vcf -p MADR_reference_popfile_d1.txt -t 98 \>
MADR_reference_d1_cloneout_98.txt
...
MADR_reference_popfile_d1.txt MADR_reference_d1_clonal_groups.txt
MADR_reference_d1_popfile_temp.txt



Visualise clonal groups in the tree using R

![](media/image67.png){width="3.6614588801399823in"
height="2.9726695100612424in"}![](media/image69.png){width="3.65834208223972in"
height="2.96875in"}

Initial results look good enough to remove these individuals from the
data and run structure.

Get the recommended clones to remove based on %missing from the cloneout
file.
...
MADR_reference_d1_clones_to_remove.txt \--recode \--out
...
Run through NJ tree building steps again for the data without clones
(See D1b)
...
#define ONEROWPERIND 0 // (B) store data for individuals in a single
line
...
#define PHASED 0 // (B) Data are in correct phase (relevant for linkage
model only)
...
#define NOTAMBIGUOUS -999 // (int) for use in some analyses of polyploid
data
...
Running snapclust and structure on d1_no_clones snapclust.R
...
![](media/image61.png){width="6.994792213473316in"
height="3.8483048993875766in"}![](media/image24.png){width="1.5677088801399826in"
height="1.3125in"}

![](media/image32.png){width="7.192708880139983in"
height="3.88878280839895in"}![](media/image27.png){width="1.5729166666666667in"
height="1.234375546806649in"}

Check to see if we get the same assignment of individuals from the
denovo and dart vcfs.

#use pop assignment script to assign populations based on structure
output

python3 \~/scripts/MADR_compare_strucout.py
MADR_reference_d1_strict_no_clones_1739223814/\*.out.csv

Compare with lineages based on NJ tree using ITOL

\### found no inconsistencies in samples beloning to minor or major
lineage assignments line up with the NJ tree for reference assembly and
for Denovo assembly



%awk -F \': \' \'{gsub(/\\(.\*\\)/, \"\", \$2); gsub(/ /, \"\", \$2);
print \$2}\' MADR_reference_d1_strict_clonal_groups.txt \>
MADR_reference_d1_strict_clonal_groups_clean.txt

\% python3 map_clones_to_popfile.py
MADR_reference_d1_strict_clonal_groups_clean.txt
MADR_reference_d1_strict_no_clones_popfile_strucK2.txt
MADR_reference_d1_strict_popfile_strucK2_with_clones.txt\
\
%awk \'{if (\$4 == \"Major_lineage\") print \$1 \>
\"MADR_reference_d1_major_lineage.txt\"; else if (\$4 ==
\"Minor_lineage\") print \$1 \>
\"MADR_reference_d1_minor_lineage.txt\"}\'
MADR_reference_d1_strict_popfile_strucK2_with_clones.txt\
\
...
### E1a_initial_genetic_sim_comparison

Remove monomorphic sites and low genotyped sites and create vcf files
for each lineage

\$ vcftools \--vcf MADR_reference_C3.vcf \--remove
MADR_reference_d1_major_lineage.txt \--recode \--out
MADR_reference_e1_minor_full_temp
...
indv_to_remove_reference_c3.txt \--recode \--out
MADR_reference_e1_minor_full_temp.vcf

#After filtering, kept 65 out of 68 Individuals

\$ vcftools \--vcf MADR_reference_C3.vcf \--remove
MADR_reference_d1_minor_lineage.txt \--recode \--out
MADR_reference_e1_major_full
...
indv_to_remove_reference_c3.txt \--recode \--out
MADR_reference_e1_major_full_temp.vcf
...
\--max-missing 0.8 \--minDP 8 \--recode \--out MADR_reference_e1_minor
...
\--max-missing 0.8 \--minDP 8 \--recode \--out MADR_reference_e1_major
...
0.8 \--minDP 8 \--recode \--out MADR_reference_e1_minor_full_filtered
...
0.8 \--minDP 8 \--recode \--out MADR_reference_e1_major_full_filtered
...
python3 /home/pbongaerts/Github/radseq/detect_clones_vcf.py -v
MADR_reference_e1_minor_full.vcf -p MADR_reference_popfile_e1_minor.txt
-o MADR_reference_e1_minor_ful_genetic_sim.txt \>
MADR_reference_e1_minor_full_clonedet_out2.txt

python3 /home/pbongaerts/Github/radseq/detect_clones_vcf.py -v
MADR_reference_e1_major_full.vcf -p MADR_reference_popfile_e1_major.txt
-o MADR_reference_e1_major_full_genetic_sim.txt \>
MADR_reference_e1_major_full_clonedet_out2.txt



Building trees for each lineage

\$ /home/pbongaerts/Github/radseq/vcf_gdmatrix.py
MADR_reference_e1_minor_full.vcf MADR_reference_popfile_e1_minor.txt \>
MADR_reference_e1_minor_full_gdmat.txt

\$ /home/pbongaerts/Github/radseq/gdmatrix2tree.py
MADR_reference_e1_minor_full_gdmat.txt MADR_reference_e1_minor_full.tre

\$ /home/pbongaerts/Github/radseq/vcf_gdmatrix.py
MADR_reference_e1_major_full.vcf MADR_reference_popfile_e1_major.txt \>
MADR_reference_e1_major_full_gdmat.txt

\$ /home/pbongaerts/Github/radseq/gdmatrix2tree.py
MADR_reference_e1_major_full_gdmat.txt MADR_reference_e1_major_full.tre



Clone detection using clone_detect.R for Major lineage with the general
filtered data

![](media/image18.png){width="5.234375546806649in"
height="2.8101213910761156in"}

Most appropriate threshold for Major lineages seems to be the Average
threshold dictated by poppr pictured in Blue in the graph above.
Important notice: the purple line indicates the genetic similarity of
the replicate pair with the highest
distance.....![](media/image30.png){width="4.848958880139983in"
height="2.525498687664042in"}

Visualise the genetic similarity between pairs in a histogram to
determine the appropriate threshold. Average similarity and average
threshold respectively for each measures seem to lie around the same
point, just after the last big peak and might be useful as a threshold.
However Using the lowest similarity between known replicates could be an
easy, more lenient way to select clones as this value lies much lower in
both cases. This would lead to a threshold of 99% similarity and a
distance threshold of 0.002748542 for the 28409 site dataset.

Comparing clone calls between the two methods:

 python3 /home/pbongaerts/Github/radseq/detect_clones_vcf.py -v
MADR_reference_e1_major.vcf -p MADR_reference_popfile_e1_major.txt -o
MADR_reference_e1_major_genetic_sim2_t99.txt -t 99 \>
MADR_reference_e1_major_clonedet_out2_t99.txt



Results based on the lineage specific filtered data:

![](media/image36.png){width="6.5in" height="3.2777777777777777in"}

![](media/image13.png){width="6.5in" height="3.2777777777777777in"}

#minor lineage

![](media/image49.png){width="6.5in" height="3.5555555555555554in"}

![](media/image56.png){width="6.5in" height="2.763888888888889in"}

Given the results we have decided to go with the lowest genetic
similarity of the replicates and set the threshold for the definition of
clones at 99% similarity
...
![](media/image25.png){width="6.5in" height="6.888888888888889in"}

With the initial threshold of 99.01% similarity we can find this
interesting laderised pattern in the NJ tree. Upon further inspection
combining the missing data we can see there is a clear overlap between
these "difficult cases" and missing data. This is likely caused by the
small denominator effect where due to the lower shared sites each
"error" carries more weight compared to complete data.

To get a better understanding of the sequencing error rate of our data
we will look into the samples sequenced multiple times by DArT.
...
\--max-missing 0.5 \--recode \--out
MADR_reference_e1_replicates_loose_filter

vcftools \--vcf MADR_reference_e1_replicates.vcf \--mac 1 \--max-missing
0.8 \--minDP 8 \--recode \--out
MADR_reference_e1_replicates_strict_filter
...
indv_to_remove_reference_e1_replicates.txt \--recode \--out
MADR_reference_e1_replicates_temp.vcf
...
MADR_reference_e1_replicates.vcf -p
MADR_reference_popfile_e1_replicates.txt -o
MADR_reference_e1_replicates_genetic_sim.txt \>
MADR_reference_e1_replicates_clonedet_out.txt

python3/home/pbongaerts/Github/radseq/vcf_clone_detect.py -v
MADR_reference_e1_replicates_strict.vcf -p
MADR_reference_popfile_e1_replicates.txt -o
MADR_reference_e1_replicates_genetic_sim_strict.txt \>
MADR_reference_e1_replicates_clonedet_strict_out.txt

python3 /home/pbongaerts/Github/radseq/vcf_clone_detect.py -v
MADR_reference_e1_replicates_loose.vcf -p
MADR_reference_popfile_e1_replicates.txt -o
MADR_reference_e1_replicates_genetic_sim_loose.txt \>
MADR_reference_e1_replicates_clonedet_loose_out.txt
...
![](media/image2.png){width="6.5in" height="3.5555555555555554in"}

![](media/image71.png){width="6.5in" height="8.916666666666666in"}

### 

### E1b_ANGSD IBS clonality assessment

Clone detection based on ANGSD IBS distance matrix



/ccg/bin/angsd -b bam_list.txt -GL 1 -uniqueOnly 1 -remove_bads 1
-minMapQ 20 -minQ 20 -minInd 475 -setMinDepth 8 -doCounts 1
-doMajorMinor 1 -doIBS 1 -makeMatrix 1 -P 1 -out
MADR_reference_e1_ANGSD_outputs
...
awk -F\'/\' \'{split(\$NF, a, \".FASTQ\"); print a\[1\]}\' bam_list.txt
\> sample_names_bam_order.txt



Initial IBS results:\
![](media/image58.png){width="6.5in" height="2.513888888888889in"}

/ccg/bin/angsd -b bam_list.txt -GL 1 -uniqueOnly 1 -remove_bads 1
-minMapQ 20 -minQ 20 -minInd 543 -setMinDepth 20 -doCounts 1
-doMajorMinor 1 -doIBS 1 -makeMatrix 1 -P 1 -out
MADR_reference_e1_ANGSD_outputs

[-\> Output filenames:]{.mark}

[-\>\"MADR_reference_e1_ANGSD_outputs.arg\"]{.mark}

[-\>\"MADR_reference_e1_ANGSD_outputs.ibs.gz\"]{.mark}

[-\>\"MADR_reference_e1_ANGSD_outputs.ibsMat\"]{.mark}

[-\> Thu Mar 13 20:56:28 2025]{.mark}

[-\> Arguments and parameters for all analysis are located in .arg
file]{.mark}

[-\> Total number of sites analyzed: 43341802]{.mark}

[-\> Number of sites retained after filtering: 1561519]{.mark}

[\[ALL done\] cpu-time used = 6030.48 sec]{.mark}

[\[ALL done\] walltime used = 6121.00 sec]{.mark}
...
-remove_bads 1 -minMapQ 20 -minQ 20 -minInd 494 -setMinDepth 20
-doCounts 1 -doMajorMinor 1 -doIBS 1 -makeMatrix 1 -P 1 -out
MADR_reference_e1_major_ANGSD_outputs

[-\> Output filenames:]{.mark}

[-\>\"MADR_reference_e1_major_ANGSD_outputs.arg\"]{.mark}

[-\>\"MADR_reference_e1_major_ANGSD_outputs.ibs.gz\"]{.mark}

[-\>\"MADR_reference_e1_major_ANGSD_outputs.ibsMat\"]{.mark}

[-\> Thu Mar 13 21:01:07 2025]{.mark}

[-\> Arguments and parameters for all analysis are located in .arg
file]{.mark}

[-\> Total number of sites analyzed: 40679307]{.mark}

[-\> Number of sites retained after filtering: 1619345]{.mark}

[\[ALL done\] cpu-time used = 5531.98 sec]{.mark}

[\[ALL done\] walltime used = 5592.00 sec]{.mark}

/ccg/bin/angsd -b bam_list_minor_lineage.txt -GL 1 -uniqueOnly 1
-remove_bads 1 -minMapQ 20 -minQ 20 -minInd 54 -setMinDepth 20 -doCounts
1 -doMajorMinor 1 -doIBS 1 -makeMatrix 1 -P 1 -out
MADR_reference_e1_minor_ANGSD_outputs

[-\> Output filenames:]{.mark}

[-\>\"MADR_reference_e1_minor_ANGSD_outputs.arg\"]{.mark}

[-\>\"MADR_reference_e1_minor_ANGSD_outputs.ibs.gz\"]{.mark}

[-\>\"MADR_reference_e1_minor_ANGSD_outputs.ibsMat\"]{.mark}

[-\> Thu Mar 13 19:32:58 2025]{.mark}

[-\> Arguments and parameters for all analysis are located in .arg
file]{.mark}

[-\> Total number of sites analyzed: 17133579]{.mark}

[-\> Number of sites retained after filtering: 1079849]{.mark}

[\[ALL done\] cpu-time used = 345.06 sec]{.mark}

[\[ALL done\] walltime used = 345.00 sec]{.mark}



#adding options to infer missingness running the same filters but not
calculating ibs to save on time

/ccg/bin/angsd -b bam_list_major_lineage.txt -GL 1 -doMajorMinor 1
-doMaf 1 -doPost 1 -doGeno 2 -doCounts 1 -uniqueOnly 1 -remove_bads 1
-minMapQ 20 -minQ 20 -minInd 494 -setMinDepth 20 -P 4 -out
geno_text_output_major

gzip -d geno_text_output_major.geno.gz

/ccg/bin/angsd -b bam_list_minor_lineage.txt -GL 1 -doMajorMinor 1
-doMaf 1 -doPost 1 -doGeno 2 -doCounts 1 -uniqueOnly 1 -remove_bads 1
-minMapQ 20 -minQ 20 -minInd 54 -setMinDepth 20 -P 4 -out
geno_text_output_minor

gzip -d geno_text_output_minor.geno.gz



Devided into clonal groups using Figure_S1_clonality.R

Initial IBS results minor lineage: setting the threshold according to
the split in node heights

![](media/image42.png){width="6.5in" height="3.2916666666666665in"}

![](media/image55.png){width="6.5in" height="2.4444444444444446in"}

Mapping the IBS clonal groups to the NJ tree (keeping colors consistent)

![](media/image17.png){width="6.5in" height="3.4444444444444446in"}

![](media/image68.png){width="6.5in" height="3.3194444444444446in"}

![](media/image38.png){width="6.5in" height="3.486111111111111in"}

![](media/image8.png){width="7.765625546806649in"
height="4.118895450568679in"}

![](media/image35.png){width="6.5in"
height="4.555555555555555in"}![](media/image1.png){width="6.5in"
height="4.097222222222222in"}

Using replicates to set a threshold:

/ccg/bin/angsd -b bam_list_replicates.txt -GL 1 -uniqueOnly 1
-remove_bads 1 -minMapQ 20 -minQ 20 -minInd 543 -setMinDepth 20
-doCounts 1 -doMajorMinor 1 -doIBS 1 -makeMatrix 1 -P 1 -out
MADR_reference_e1_replicates_ANGSD_outputs

[-\> Output filenames:]{.mark}

[-\>\"MADR_reference_e1_replicates_ANGSD_outputs.arg\"]{.mark}

[-\>\"MADR_reference_e1_replicates_ANGSD_outputs.ibs.gz\"]{.mark}

[-\>\"MADR_reference_e1_replicates_ANGSD_outputs.ibsMat\"]{.mark}

[-\> Sun Mar 16 16:29:51 2025]{.mark}

[-\> Arguments and parameters for all analysis are located in .arg
file]{.mark}

[-\> Total number of sites analyzed: 43342207]{.mark}

[-\> Number of sites retained after filtering: 1504952]{.mark}

[\[ALL done\] cpu-time used = 7326.38 sec]{.mark}

[\[ALL done\] walltime used = 7372.00 sec]{.mark}

#Setting threshold to 0.016 based on the lowest scoring replicate
cluster ibs.

![](media/image34.png){width="7.345321522309711in"
height="2.8839807524059493in"}

Find the threshold for each individually:

/ccg/bin/angsd -b bam_list_replicates_minor_lineage.txt -GL 1
-uniqueOnly 1 -remove_bads 1 -minMapQ 20 -minQ 20 -minInd 66
-setMinDepth 20 -doCounts 1 -doMajorMinor 1 -doIBS 1 -makeMatrix 1 -P 1
-out MADR_reference_e1_replicates_minor_ANGSD_outputs

[-\> Output filenames:]{.mark}

[-\>\"MADR_reference_e1_replicates_minor_ANGSD_outputs.arg\"]{.mark}

[-\>\"MADR_reference_e1_replicates_minor_ANGSD_outputs.ibs.gz\"]{.mark}

[-\>\"MADR_reference_e1_replicates_minor_ANGSD_outputs.ibsMat\"]{.mark}

[-\> Mon Mar 17 14:05:00 2025]{.mark}

[-\> Arguments and parameters for all analysis are located in .arg
file]{.mark}

[-\> Total number of sites analyzed: 17129578]{.mark}

[-\> Number of sites retained after filtering: 1270478]{.mark}

[\[ALL done\] cpu-time used = 522.16 sec]{.mark}

[\[ALL done\] walltime used = 523.00 sec]{.mark}

/ccg/bin/angsd -b bam_list_replicates_major_lineage.txt -GL 1
-uniqueOnly 1 -remove_bads 1 -minMapQ 20 -minQ 20 -minInd 563
-setMinDepth 20 -doCounts 1 -doMajorMinor 1 -doIBS 1 -makeMatrix 1 -P 1
-out MADR_reference_e1_replicates_major_ANGSD_outputs

[-\> Output filenames:]{.mark}

[-\>\"MADR_reference_e1_replicates_major_ANGSD_outputs.arg\"]{.mark}

[-\>\"MADR_reference_e1_replicates_major_ANGSD_outputs.ibs.gz\"]{.mark}

[-\>\"MADR_reference_e1_replicates_major_ANGSD_outputs.ibsMat\"]{.mark}

[-\> Mon Mar 17 15:44:52 2025]{.mark}

[-\> Arguments and parameters for all analysis are located in .arg
file]{.mark}

[-\> Total number of sites analyzed: 40677711]{.mark}

[-\> Number of sites retained after filtering: 1621212]{.mark}

[\[ALL done\] cpu-time used = 6412.29 sec]{.mark}

[\[ALL done\] walltime used = 6420.00 sec]{.mark}
...
# F1_lineage_comparison

Create new datasets without clonal samples:

vcftools \--vcf MADR_reference_C3.vcf \--keep
MADR_reference_e1_major_samples_to_keep.txt \--recode \--out
MADR_reference_f1_major_full

[VCFtools - 0.1.16]{.mark}

[Keeping individuals in \'keep\' list]{.mark}

[After filtering, kept 177 out of 681 Individuals]{.mark}

[Outputting VCF file\...]{.mark}

[After filtering, kept 418633 out of a possible 418633 Sites]{.mark}

[Run Time = 196.00 seconds]{.mark}

vcftools \--vcf MADR_reference_C3.vcf \--keep
MADR_reference_e1_minor_samples_to_keep.txt \--recode \--out
MADR_reference_f1_minor_full

[Keeping individuals in \'keep\' list]{.mark}

[After filtering, kept 28 out of 681 Individuals]{.mark}
...
vcftools \--vcf MADR_reference_C3.vcf \--keep
MADR_reference_e1_all_samples_to_keep.txt \--recode \--out
MADR_reference_f1_full

[Keeping individuals in \'keep\' list]{.mark}

[After filtering, kept 205 out of 681 Individuals]{.mark}

[Outputting VCF file\...]{.mark}

[After filtering, kept 418633 out of a possible 418633 Sites]{.mark}

[Run Time = 205.00 seconds]{.mark}
...
0.8 \--minDP 8 \--recode \--out MADR_reference_f1_major
...
0.8 \--minDP 8 \--recode \--out MADR_reference_f1_minor
...
\--minDP 8 \--recode \--out MADR_reference_f1_full
...
\--max-missing 0.05 \--minDP 8 \--recode \--out
MADR_reference_f1_full_loose_filter
...
\$ sort -gk 5 -o MADR_reference_f1_major_qc.txt
MADR_reference_f1_major_qc.txt
...
[Samples with \<10% genotyped: 0]{.mark}

[Samples with \<30% genotyped: 2]{.mark}

[Samples with \<1000 SNPs (GENO): 0]{.mark}

[Average % genotyped: 90.1801]{.mark}

#repeat:\
\' MADR_reference_f1_minor_qc.txt

[Samples with \<10% genotyped: 1]{.mark}

[Samples with \<30% genotyped: 1]{.mark}

[Samples with \<1000 SNPs (GENO): 1]{.mark}

[Average % genotyped: 87.1186]{.mark}

#Remove 1 indv

[\' MADR_reference_f1_all_qc.txt]{.mark}

[Samples with \<10% genotyped: 1]{.mark}

[Samples with \<30% genotyped: 3]{.mark}

[Samples with \<1000 SNPs (GENO): 1]{.mark}

[Average % genotyped: 89.4151]{.mark}
...
\$ /home/pbongaerts/Github/radseq/vcf_gdmatrix.py
MADR_reference_f1_major.vcf MADR_reference_popfile_f1_major.txt \>
MADR_reference_f1_major_gd_mat.txt
...
MADR_reference_e1_major_samples_to_keep.txt \--freq \--out
allele_frequencies/major_lineage_allele_freq

\# For minor lineage

vcftools \--vcf MADR_reference_f1_full_loose_filter.recode.vcf \--keep
MADR_reference_e1_minor_samples_to_keep.txt \--freq \--out
allele_frequencies/minor_lineage_allele_freq
...
vcftools \--vcf
../hybrids_not_separated/MADR_reference_f1_full_loose_filter.recode.vcf
\--keep DAPC_major_samples_to_keep.txt \--freq \--out
major_lineage_allele_freq
...
vcftools \--vcf
../hybrids_not_separated/MADR_reference_f1_full_loose_filter.recode.vcf
\--keep DAPC_minor_samples_to_keep.txt \--freq \--out
minor_lineage_allele_freq
...
vcftools \--vcf
../hybrids_not_separated/MADR_reference_f1_full_loose_filter.recode.vcf
\--keep DAPC_hybrid_samples_to_keep.txt \--freq \--out
hybrids_allele_freq
...
###  F1b_Population_statistical_analysis
...
MADR_reference_popfile_f1_all.txt

\> MADR_reference_popfile_f1_lineages.txt



Running popstats_MADR.R

#PCA with NA = 'mean'\
![](media/image4.png){width="6.5in" height="3.763888888888889in"}%%\
![](media/image66.png){width="6.5in" height="3.7083333333333335in"}

![](media/image29.png){width="6.5in" height="3.7083333333333335in"}

DAPC

![](media/image19.png){width="6.5in"
height="3.736111111111111in"}![](media/image26.png){width="3.244792213473316in"
height="1.8667957130358706in"}

![](media/image60.png){width="5.588542213473316in"
height="3.2096358267716534in"}

Following the DAPC posterior assignment:

![](media/image16.png){width="6.5in" height="3.736111111111111in"}

![](media/image48.png){width="6.5in" height="3.736111111111111in"}

![](media/image15.png){width="6.5in" height="3.5833333333333335in"}
...
[Initialise indivs and pops for ../MADR_reference_f1_all.vcf\...]{.mark}

[Subsample SNPs (one random SNP per locus)\... \[5492
SNPs/loci\]]{.mark}

[Outputting 10 STRUCTURE files\...10 reps DONE]{.mark}

[Executing 32 parallel STRUCTURE runs for K = 2 \...10 reps DONE]{.mark}

[Executing 32 parallel STRUCTURE runs for K = 3 \...10 reps DONE]{.mark}

[Executing 32 parallel STRUCTURE runs for K = 4 \...10 reps DONE]{.mark}

[Running CLUMPP on replicates for K = 2 \...]{.mark}

[Running CLUMPP on replicates for K = 3 \...]{.mark}

[Running CLUMPP on replicates for K = 4 \...]{.mark}

[K = 2: MedMeaK 1.0 MaxMeaK 1 MedMedK 1.0 MaxMedK 1
../MADR_reference_f1_all.vcf]{.mark}
...
height="3.6805555555555554in"}![](media/image64.png){width="1.640625546806649in"
height="1.0883880139982502in"}
...
![](media/image40.png){width="6.5in" height="2.8333333333333335in"}

MAX PCAs= 106![](media/image59.png){width="3.875in"
height="3.6994772528433946in"}![](media/image62.png){width="3.8726891951006124in"
height="3.6927088801399823in"}

MAX PCAs =2 (more similar to results without added
hybrids)![](media/image52.png){width="3.875in"
height="3.6967027559055117in"}![](media/image54.png){width="3.875in"
height="3.7037292213473316in"}

![](media/image10.png){width="6.5in" height="3.3194444444444446in"}

"Suspected" hybrids in green
PCA![](media/image31.png){width="3.614024496937883in"
height="3.4448950131233596in"}

![](media/image28.png){width="3.5404768153980752in"
height="3.3802088801399823in"}

![](media/image3.png){width="6.5in" height="6.194444444444445in"}
...
![](media/image51.png){width="6.5in" height="3.125in"}

![](media/image6.png){width="6.5in" height="3.125in"}

![](media/image43.png){width="6.5in" height="3.2777777777777777in"}

Nj tree

It still seems like there is a fair bit of noise which could be due to
the selection of samples to hybridize.

![](media/image37.png){width="6.5in" height="3.5555555555555554in"}

HC clust

![](media/image70.png){width="6.5in" height="3.4305555555555554in"}

NJ tree

![](media/image14.png){width="6.5in" height="3.388888888888889in"}

Full SIM data with only pure hybrids

![](media/image12.png){width="6.5in" height="3.2777777777777777in"}

![](media/image50.png){width="6.5in" height="2.9305555555555554in"}

![](media/image73.png){width="6.5in" height="3.5972222222222223in"}

![](media/image44.png){width="6.5in" height="3.4583333333333335in"}

### ![](media/image46.png){width="6.5in" height="5.722222222222222in"}
...
### F1c_genetic_distance

Genetic distance between lineages was calculated using hierfstat in the
Popstants_full_data.R

Script.

![](media/image41.png){width="4.348958880139983in"
height="2.5133595800524935in"}

  ------------------------------------------------------------------
  **Stat**   **Meaning**      **Interpretation**
  ---------- ---------------- --------------------------------------
  **Ho**     Observed         Mean proportion of individuals that
             heterozygosity   are *heterozygous* at loci across the
                              populations. (Here: **0.0998**)

  **Hs**     Expected         Expected heterozygosity assuming
             heterozygosity   Hardy-Weinberg within each
             within           *population*. (Here: **0.1383**)
             populations      

  **Ht**     Total expected   Expected heterozygosity if you treated
             heterozygosity   *all populations together* as one big
                              group. (Here: **0.1499**)

  **Dst**    Genetic          How much of the total genetic
             divergence       diversity is **due to differences
             between          between populations** (Ht - Hs).
             populations      (Here: **0.0115**)

  **Htp**    Ht corrected for A corrected version of Ht to account
             sampling size    for unequal or small sample sizes.
                              (Here: **0.1557**)

  **Dstp**   Dst corrected    Same correction for Dst. (Here:
             for sampling     **0.0174**)

  **Fst**    Unbiased Weir &  Proportion of genetic variation **due
             Cockerham global to differences between populations**.
             Fst              (Here: **0.0770**, or 7.7%)

  **Fstp**   Fst corrected    A corrected Fst. (Here: **0.1117**)
             for sampling     
             size             

  **Fis**    Inbreeding       Measures *departure from
             coefficient      Hardy-Weinberg* inside populations:
             within           **positive = excess of homozygosity**.
             populations      (Here: **0.2786**)

  **Dest**   Jost's D, an     A measure of *actual allele
             alternative to   differentiation* between pops. (Here:
             Fst              **0.0202**)
  ------------------------------------------------------------------

# F2_PCadapt

From [MADR_pcadapt.R]{.underline}

[pcadapt]{.underline}

![](media/image11.png){width="6.5in" height="5.958333333333333in"}

# ![](media/image20.png){width="6.5in" height="5.722222222222222in"}

![](media/image65.png){width="6.5in" height="6.527777777777778in"}

![](media/image74.png){width="6.5in" height="6.180555555555555in"}

RDA

![](media/image7.png){width="6.5in" height="3.7222222222222223in"}

![](media/image21.png){width="6.5in" height="3.5694444444444446in"}

# 

# G1_reefscape_assessment

G1a_demographic analysis of stands

monstand_metrics.r

### G1b_genotypic_diversity_stands

monostand_diversity.R

### G1c_genetic_diversity_stands

To DO:

Genotypes per monostand

Distribution of genotypes within and among stands
